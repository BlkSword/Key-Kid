...
def xor_repeating_break(data: str, encoding: str = "hex", min_key: int = 2, max_key: int = 40) -> BreakResult:
    b = _parse_data(data, encoding)
    candidates = []
    ...
    for ks, _ in candidates[:5]:
        ...
        if sc > best_score:
            ...
            best_pt = txt
            best_key = bytes(key)
    return BreakResult(algorithm="XOR-repeating", plaintext=best_pt, key=best_key.decode(errors="ignore"), confidence=best_score)
