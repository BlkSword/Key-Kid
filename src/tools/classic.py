...
def vigenere_break(ciphertext: str, max_key_len: int = 16, top_k: int = 3) -> list[BreakResult]:
    ...
    return [BreakResult(algorithm="Vigenere", plaintext=pt, key=key, confidence=sc) for key, sc, pt in cands[:top_k]]
...
def affine_break(ciphertext: str, top_k: int = 3) -> list[BreakResult]:
    ...
    for a in range(1, 26):
        ...
        for b in range(26):
            ...
            pt = ''.join(res)
            sc = english_score(pt)
            out.append(BreakResult(algorithm="Affine", plaintext=pt, key=f"a={a},b={b}", confidence=sc))
...
def transposition_break(ciphertext: str, max_key_len: int = 5, top_k: int = 3) -> list[BreakResult]:
    ...
    for k in range(2, max_key_len + 1):
        for perm in itertools.permutations(range(k)):
            ...
            sc = english_score(pt)
            out.append(BreakResult(algorithm="Transposition", plaintext=pt, key='-'.join(str(x) for x in perm), confidence=sc))
    out.sort(key=lambda x: x.confidence, reverse=True)
    return out[:top_k]
